...
import { Globe, ArrowLeft, HardHat, ChevronRight } from "lucide-react";
import Link from 'next/link';

// Wiki system language type
type WikiLanguage = 'en' | 'fr';

export default function StaffWikiPage() {
    ...
    <div className="container mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h1 className="text-3xl font-bold tracking-tight">
          {language === 'en' ? 'Staff Member User Guide' : 'Guide Utilisateur - Membre du Personnel'}
        </h1>
        <Badge variant="outline" className="mt-2 md:mt-0 text-sm md:text-base px-3 py-1 border-primary text-primary">
          {language === 'en' ? 'ROLE: STAFF' : 'RÔLE: PERSONNEL'}
        </Badge>
      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-col items-center justify-center text-center p-6">
                <HardHat className="h-16 w-16 text-yellow-600 mb-4" />
                <h2 className="text-xl font-medium mb-2">
                  {language === 'en' ? 'Coming Soon' : 'Bientôt disponible'}
                </h2>
                <p className="text-muted-foreground max-w-md">
                  {language === 'en' 
                    ? 'The Staff Member wiki page is currently under development. Please check back later for comprehensive documentation on staff roles and responsibilities.'
                    : 'La page wiki du Membre du Personnel est actuellement en développement. Veuillez revenir plus tard pour une documentation complète sur les rôles et responsabilités du personnel.'}
                </p>
              </div>
            </CardContent>
          </Card>
